...
def parse_source_and_create_output(source_name, source_file, output_type, output_file):
	...
	# fro each line we want another query,
	# so create json dump for each ip:port-ip:port couple
    for line in source.readlines()[1:]:
        output = line.split("\n")[0]
    	# JSON Part
    	json_dict = {'source_name' : source_name, 'update_time' : update_time, 'output_type' : output_type, 'output' : output, 'protocol' : 'tcp', 'bytes':17000}
